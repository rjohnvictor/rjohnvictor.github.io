...
import styles from './KiCursor.module.css';
...
export default function KiCursor() {
    ...
    useEffect(() => {
        ...
        noHoverQuery.addEventListener?.('change', recompute);

        return () => {
            window.removeEventListener('resize', recompute);
            coarseQuery.removeEventListener?.('change', recompute);
            noHoverQuery.removeEventListener?.('change', recompute);
        };
    }, []);
    ...
    useEffect(() => {
        ...
        const onMove = (e: MouseEvent) => {
            mouseX = e.clientX;
            mouseY = e.clientY;
            isHover = !!(e.target as HTMLElement)?.closest?.(
    ...
    return (
        <canvas
            ...
            style={{
                display: theme === 'power' && !isMobileLike ? 'block' : 'none',
            }}
        />
    );
}
